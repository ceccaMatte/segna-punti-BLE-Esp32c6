...
static inline uint8_t wearable_token(uint8_t button_index, wearable_primitive_t primitive)
{
    return (uint8_t)(((button_index & 0x03u) << 2) | ((uint8_t)primitive & 0x03u));
}

static inline uint8_t wearable_token_button(uint8_t token)
{
    return (uint8_t)((token >> 2) & 0x03u);
}

static inline wearable_primitive_t wearable_token_primitive(uint8_t token)
{
    return (wearable_primitive_t)(token & 0x03u);
}

typedef struct {
    uint8_t length;
    uint8_t tokens[WEARABLE_MAX_SEQUENCE];
} wearable_sequence_t;
